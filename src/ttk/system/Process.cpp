...
namespace ttk::Process {
    ...
    namespace {
        ...
        std::string describe(const int number) {
            return std::generic_category().message(number);
        }

        void close_all(const std::initializer_list<int> ends) {
        ...
        bool open_terminal(int (&ends)[2]) {
            ...
            constexpr winsize size{.ws_row = 24, .ws_col = 80, .ws_xpixel = 0, .ws_ypixel = 0};
